...
import (
	...
	"kubevirt.io/kubevirt-migration-controller/test/utils/watcher"
)
...
func WaitForVMIPhase(
	vmi *v1.VirtualMachineInstance,
	client client.Client,
	phases []v1.VirtualMachineInstancePhase, opts ...Option) *v1.VirtualMachineInstance {
	wp := watcher.WarningsPolicy{FailOnWarnings: true}
	gomega.ExpectWithOffset(1, phases).ToNot(gomega.BeEmpty())
	waiting := Waiting{timeout: defaultTimeout, wp: &wp, phases: phases, client: client}
	for _, f := range opts {
		f(&waiting)
	}
...
// If no warning policy has been set before, a new one will be set implicitly with fail on warnings
// enabled and the ignore list added
...
// watchVMIForPhase looks at the vmi object and, after it is started, waits for it to satisfy the phases
...
func (w *Waiting) watchVMIForPhase(vmi *v1.VirtualMachineInstance) *v1.VirtualMachineInstance {
	...
	if vmi.ResourceVersion == "" {
		err := w.client.Get(context.Background(), client.ObjectKey{Namespace: vmi.Namespace, Name: vmi.Name}, vmi)
	...
	gomega.EventuallyWithOffset(1, func(g gomega.Gomega) v1.VirtualMachineInstancePhase {
		retrievedVMI = &v1.VirtualMachineInstance{}
		err := w.client.Get(context.Background(), client.ObjectKey{Namespace: vmi.Namespace, Name: vmi.Name}, retrievedVMI)
		gomega.ExpectWithOffset(1, err).ToNot(gomega.HaveOccurred())
...
func WaitForSuccessfulVMIStart(
	vmi *v1.VirtualMachineInstance,
	cli client.Client, opts ...Option) *v1.VirtualMachineInstance {
	return WaitForVMIPhase(vmi, cli, []v1.VirtualMachineInstancePhase{v1.Running}, opts...)
}
...
func WaitUntilVMIReady(vmi *v1.VirtualMachineInstance, cli client.Client,
	loginTo console.LoginToFunction, opts ...Option) *v1.VirtualMachineInstance {
	vmi = WaitForVMIPhase(vmi, cli, []v1.VirtualMachineInstancePhase{v1.Running}, opts...)
	...
	err = cli.Get(context.Background(), client.ObjectKey{Namespace: vmi.Namespace, Name: vmi.Name}, vmi)
	gomega.Expect(err).ToNot(gomega.HaveOccurred())

	return vmi
}

// WaitForVirtualMachineToDisappearWithTimeout blocks for the passed seconds until the specified
// VirtualMachineInstance disappears
func WaitForVirtualMachineToDisappearWithTimeout(vmi *v1.VirtualMachineInstance, cli client.Client, seconds int) {
...
// WaitForMigrationToDisappearWithTimeout blocks for the passed duration until the specified
// VirtualMachineInstanceMigration disappears
func WaitForMigrationToDisappearWithTimeout(
	...
	gomega.EventuallyWithOffset(1, func() error {
		...
		return err
	}, timeout, 1*time.Second).Should(gomega.SatisfyAll(
		gomega.HaveOccurred(),
		gomega.WithTransform(errors.IsNotFound, gomega.BeTrue())),
		"The migration should be gone within the given timeout")
	return nil
}
